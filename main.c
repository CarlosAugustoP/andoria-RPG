...
void inserir(Arv **t, char texto [3500], int id, int combatFlag, int karma);
int game();
void clear();
...
void inserir(Arv **t, char* texto, int id, int combatFlag, int karma) {
    if (*t == NULL) {
        *t = (Arv *)malloc(sizeof(Arv));
        ...
        if (id < (*t)->id) {
            inserir(&(*t)->esq, texto,id,combatFlag, karma);
        }
        if (id > (*t)->id) {
            inserir(&(*t)->dir,texto, id,combatFlag, karma);
        }
    }
}
